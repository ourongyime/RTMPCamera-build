...
class MainViewController: UIViewController {
    ...
    private func startVideoProcessing() {
        ...
        case .rtmpStream:
            addLog("RTMP: \(rtmpURL)")
            addLog("(RTMP 需要集成 FFmpeg/librtmp 库)")
        case .localVideo:
            guard !localVideoPath.isEmpty else { addLog("请先选择视频文件"); return }
            startLocalVideoPlayback()
        }
        writeMetaFile()
    }

    private func stopVideoProcessing() {
    ...
    private func startLocalVideoPlayback() {
        ...
        do {
            ...
            addLog("开始播放: \((localVideoPath as NSString).lastPathComponent)")
            
            // 定时读取帧并写入文件
            videoPlaybackTimer = Timer.scheduledTimer(withTimeInterval: 1.0/30.0, repeats: true) { [weak self] _ in
                self?.processNextFrame()
            }
        } catch {
            addLog("视频读取失败: \(error.localizedDescription)")
        }
    }

    private func processNextFrame() {
        guard let output = videoReaderOutput else { return }
        if let sampleBuffer = output.copyNextSampleBuffer() {
            if let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                writePixelBufferToFile(pixelBuffer)
            }
        } else if videoReader?.status == .completed, loopEnabled {
            // 循环播放
            stopVideoProcessing()
            startLocalVideoPlayback()
        }
    ...
    private func writePixelBufferToFile(_ pixelBuffer: CVPixelBuffer) {
        ...
        try? FileManager.default.createDirectory(atPath: frameOutputDir, withIntermediateDirectories: true, attributes: nil)
        let data = Data(bytes: baseAddress, count: dataSize)
        try? data.write(to: URL(fileURLWithPath: frameFile), options: .atomic)
    }

    private func writeMetaFile() {
        var meta: [String: Any] = [
            ...
            "frameWidth": 640,
            "frameHeight": 480,
            "frameBytesPerRow": 640 * 4
        ]
        try? FileManager.default.createDirectory(atPath: frameOutputDir, withIntermediateDirectories: true, attributes: nil)
        if let data = try? PropertyListSerialization.data(fromPropertyList: meta, format: .xml, options: 0) {
            try? data.write(to: URL(fileURLWithPath: metaFile), options: .atomic)
        }
    }

    private func sendControlCommand() {
        let sourceNames = ["真实摄像头", "RTMP推流", "本地视频"]
        addLog("应用: \(sourceNames[currentSource.rawValue]) 注入视频=\(videoInjectionOn ? "开" : "关") 音频=\(audioInjectionOn ? "开" : "关") 循环=\(loopEnabled ? "开" : "关")")
        ...
        // 启动视频处理（app 内直接处理，输出帧到文件供 tweak 读取）
